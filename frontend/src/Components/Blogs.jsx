...
function Blogs() {
    ...
    useEffect(() => {
        ...
        auth.onAuthStateChanged((user) => {
            if (user)
            {
              if(user.uid === "L1ybz2jKAzRIv6H4wpLqueGdx3t1")
                {
                    setAdmin(true)
                }
                else{
    ...
    const handleLike = async (blog_id) => {
        try {
            const response = await axios.patch(`${baseUrl}/api/blogs/like/${blog_id}`);
    ...
    const handleNewBlogSubmit = (event) => {
        ...
        const likes = 0
        axios.post(`${baseUrl}/api/blogs`, { newTitle, date, newContent, likes }).then((res) => {
            console.log(res.data)
